...
As we discussed in our blog  difficulty effects the gaming experience. In short, skilled players prefer the game harder while new comers in the genre would rather have easier experience, see <https://science.goodjobgmaes.com> for more.
...
input_list <- list(
  ...
  level_id = train_data$level  - 100,
  N_of_level_id = max(train_data$level) - 100,
...
test_users_simulated_try_model_2 <- get_simulated_retry(
  fit$output_files(), 
  MIN_TEST_USER_ID, 
  MIN_TEST_LEVEL_ID
)
